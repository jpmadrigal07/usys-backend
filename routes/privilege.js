  ...
  if (!isNil(desc)) {
    ...
    try {
      ...
      if (getPrivilege.length === 0) {
        ...
        res.json(createPrivilege);
      } else {
        res.status(500).json("Privilege is already in use");
      }
    } catch ({ message: errMessage }) {
      const message = errMessage ? errMessage : UNKNOWN_ERROR_OCCURED;
      res.status(500).json(message);
...
router.put("/:id", async (req, res) => {
  ...
  if (!isNil(desc)) {
    try {
      ...
      if (getPrivilege.length === 0) {
        const updatePrivilege = await Privilege.findByIdAndUpdate(
          req.params.id,
          {
            $set: {
              desc,
...
router.patch("/:id", async (req, res) => {
  ...
  if (!isEmpty(condition)) {
    try {
      ...
      res.json(updatePrivilege);
    } catch ({ message: errMessage }) {
      const message = errMessage ? errMessage : UNKNOWN_ERROR_OCCURED;
      res.status(500).json(message);
    }
  } else {
    res.status(500).json("Privilege Cannot be found");
  }
});
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (getPrivilege.length > 0) {
      ...
      res.status(500).json("Privilege is already deleted");
    }
  } catch ({ message: errMessage }) {
    const message = errMessage ? errMessage : UNKNOWN_ERROR_OCCURED;
    res.status(500).json(message);
